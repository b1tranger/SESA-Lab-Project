<?php
// This is the complete PHP logic from old.registration_form.php
include("../connection.php");

$form_error = '';
$form_success = false;

if (isset($_POST['submit'])) { // Logic from old.registration_form.php
    $username = trim($_POST['input_name']);
    $type = $_POST['input_type'];
    $email = trim($_POST['input_email']);
    $password = trim($_POST['input_password']);
    $confirm = trim($_POST['input_password2']);

    if (empty($username) || empty($type) || empty($email) || empty($password) || empty($confirm)) {
        $form_error = "Please fill out all fields.";
    } else if ($password != $confirm) {
        $form_error = "Passwords do not match. Please try again.";
    } else {
        // Check if email already exists
        $sql_check = "select email from account where email='$email'";
        $res_check = mysqli_query($conn, $sql_check);
        if (mysqli_num_rows($res_check) > 0) {
            $form_error = "An account with this email already exists. <a href='login.php' style='color: #ffc0c0; text-decoration: underline;'>Login instead?</a>";
        } else {
            // passing to the Database
            $sql = "insert into account(email, password, username, type) values('$email','$password','$username','$type')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
                $form_success = true;
                // header("location:login.php"); // Original logic had this commented out, so I'm keeping it that way.
            } else {
                $form_error = "An error occurred. Please try again later.";
            }
        }
    }
}
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Register - Support Hero</title> <!-- Title from old.registration_form.php -->
    <link rel="icon" href="Images/techimage.jpg" type="image">
    <link href="https://fonts.googleapis.com/css2?family=Jost:wght@500&display=swap" rel="stylesheet" />

    <!-- CSS from registration.php -->
    <style>
        body {
            margin: 0;
            padding: 0;
            display: flex;
            justify-content: center;
            align-items: center;
            min-height: 100vh;
            font-family: 'Jost', sans-serif;
            background: linear-gradient(to bottom, #00416A, #E4E5E6);
        }

        .main {
            width: 370px;
            /* Height set to auto to accommodate all fields and error messages */
            height: auto;
            height: 520px;max-height: 520px;
            overflow: hidden;
            background: #fff url("background.jpg") no-repeat center/cover;
            border-radius: 10px;
            box-shadow: 5px 20px 50px rgba(0, 0, 0, 0.3);
            position: relative;
            padding-bottom: 20px;
        }

        #chk {
            display: none;
        }

        .signup {
            position: relative;
            width: 100%;
            height: 100%;
            padding-bottom: 20px; /* Added padding */
        }

        label {
            color: #000;
            font-size: 2em;
            display: flex;
            justify-content: center;
            margin: 30px; /* Margin from registration.php */
            font-weight: bold;
            cursor: pointer;
            transition: 0.5s ease-in-out;
        }

        input {
            width: 60%;
            background: #e0dede;
            display: flex;
            justify-content: center;
            margin: 15px auto;
            padding: 10px;
            border: none;
            outline: none;
            border-radius: 5px;
        }

        select {
            width: 65%; /* Width from registration.php */
            background: #e0dede;
            justify-content: center;
            display: flex;
            margin: 10px auto;
            padding: 10px; /* Adjusted padding to match inputs */
            border: none;
            outline: none;
            border-radius: 5px;
            color: #333;
            font-size: 0.9em; /* Adjusted font size */
            font-family: 'Jost', sans-serif; /* Ensure font consistency */
        }

        button {
            width: 50%;
            height: 40px;
            margin: 20px auto; /* Margin from registration.php */
            display: block;
            color: #fff;
            background: #00416A;
            font-size: 1em;
            font-weight: bold;
            border: none;
            border-radius: 5px;
            cursor: pointer;
            transition: 0.3s ease-in;
        }

        button:hover {
            background: #026AA7;
        }

        .home {
            height: 500px;
            background: #eee;
            border-radius: 60% / 10%;
            transform: translateY(-115px); /* Transform from registration.php */
            transition: 0.6s ease-in-out;
        }

        .home a {
            text-decoration: none;
            color: #00416A;
            transform: scale(0.6);
        }

        .home a:hover {
            color: red;
            text-decoration: overline;
            cursor: pointer;
        }

        p {
            text-align: center;
        }

        /* Style for the error message (from old.registration_form.php) */
        .form-error {
            background-color: #5a2a2a;
            color: #ffc0c0;
            padding: 1rem;
            border-radius: 5px;
            text-align: center;
            margin: 10px auto;
            width: 65%;
            font-size: 0.9em;
        }
    </style>
</head>

<body>
    <div class="main">

        <?php 
        // Logic from old.registration_form.php
        if ($form_success) { 
        ?>
            <!-- Success Message -->
            <div class="signup">
                <label for="chk" aria-hidden="true" style="margin-bottom: 10px;">Registration Successful!</label>
                <p style="text-align: center; color: #333; font-size: 1.1em; padding: 0 20px;">
                    Your account has been created.
                </p>
                <p style="text-align: center; font-size: 1em; margin-top: 20px;">
                    <a href="login.php" style="color: #00416A; text-decoration: underline;">Click here to Login</a>
                </p>
            </div>

        <?php 
        } else { 
        ?>
            <!-- Registration Form -->
            <div class="signup">
                <!-- HTML from registration.php, but form tag is empty -->
                <form method="POST">
                    <label for="chk" aria-hidden="true">Registration</label>

                    <?php 
                    // Error message logic from old.registration_form.php
                    if (!empty($form_error)) { 
                    ?>
                        <div class="form-error">
                            <?php echo $form_error; ?>
                        </div>
                    <?php } ?>

                    <!-- 
                        Inputs from registration.php
                        Input names MUST match old.registration_form.php logic
                    -->
                    <input type="text" name="input_name" placeholder="Name" required />
                    <select id="input_type" name="input_type" required>
                        <option value="">-- Select your role --</option>
                        <option value="provider">Provider</option>
                        <option value="consumer">Consumer</option>
                    </select>
                    <input type="email" name="input_email" placeholder="Email" required />
                    <input type="password" name="input_password" placeholder="Password" required />
                    <input type="password" name="input_password2" placeholder="Confirm Password" required />

                    <!-- 
                        Button name MUST be "submit" to match old.registration_form.php
                        (if (isset($_POST['submit'])))
                    -->
                    <button type="submit" name="submit">Create Account</button>

                    <!-- Link from old.registration_form.php -->
                    <div style="text-align: center; font-size: 0.9rem;">
                        <p>Already have an account? <a href="login.php" style="color: #00416A;">Login here</a></p>
                    </div>
                </form>
            </div>

            <!-- Home Section from registration.php -->
            <div class="home">
                <form>
                    <label for="chk" aria-hidden="true">
                        <a href="../Home_Page/index.php">Home</a>
                    </label>
                </form>
            </div>
        <?php } ?>
    </div>
</body>

</html>